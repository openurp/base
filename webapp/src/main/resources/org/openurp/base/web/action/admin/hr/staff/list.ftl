...
[@b.grid items=staffs var="staff"]
  ...
  [@b.row]
    ...
    [@b.col width="14%" property="staffType.name" title="类别"]${(staff.staffType.name)!}[/@]
    [@b.col width="12%" property="degree.name" title="学位"]
      [#if staff.degree??]${staff.degree.name?replace("学位","")}[#else]--[/#if]
    [/@]
    [@b.col width="9%" property="title.name" title="职称"]${(staff.title.name)!}[/@]
    [@b.col width="7%" property="status.name" title="在职状态"/]
