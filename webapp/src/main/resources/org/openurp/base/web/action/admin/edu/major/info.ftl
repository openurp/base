  ...
  <div class="card card-info card-primary card-outline">
    ...
    <table class="table table-sm table-detail">
      ...
      <tr>
        ...
        <td>${major.shortName!}</td>
      </tr>
      <tr>
        <td class="title">学科信息</td>
        <td>[#list major.disciplines as md]${md.category.name} ${md.disciplineCode!"--"} [#if md_has_next]&nbsp;[/#if][/#list]</td>
        <td class="title">学制</td>
        <td>[#list major.schoolLengths as sl]${sl.level.name}(${sl.fromGrade}~${sl.toGrade!}) ${sl.normal}年(${sl.minimum}~${sl.maximum}) [#if sl_has_next]<br>[/#if][/#list]</td>
      </tr>
      <tr>
        <td class="title">有效期</td>
        <td >${major.beginOn}~${major.endOn!"现在"}</td>
