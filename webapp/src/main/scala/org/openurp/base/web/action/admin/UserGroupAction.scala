...
class UserGroupAction extends RestfulAction[UserGroup] with SchoolSupport {
  ...
  override protected def editSetting(entity: UserGroup): Unit = {
    val parents = entityDao.findBy(classOf[UserGroup], "school", getSchool).toBuffer
    if (entity.persisted) {
  ...
  override protected def saveAndRedirect(entity: UserGroup): View = {
