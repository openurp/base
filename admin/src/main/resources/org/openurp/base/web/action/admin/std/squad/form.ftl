...
[@b.toolbar title="行政班级信息"]bar.addBack();[/@]
  [@b.form action=sa theme="list" action=b.rest.save(squad)]
    ...
    [@b.select name="squad.level.id" label="培养层次" value=squad.level! required="true"  items=levels option="id,name" empty="..."/]
    [@b.select name="squad.eduType.id" label="培养类型" value=squad.eduType! required="true" items=eduTypes empty="..."/]
    [@b.select name="squad.stdType.id" label="学生类别" value=squad.stdType! required="true" items=stdTypes empty="..."/]
    ...
    [@b.select name="squad.mentor.id" label="辅导员" value=squad.mentor!
               style="width:300px;" href=urp.api+"/base/staffs.json?q={term}" option="id,description" empty="..."/]
