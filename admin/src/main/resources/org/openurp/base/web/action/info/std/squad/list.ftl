    ...
    <div class="card-body">
        <table class="table table-hover table-sm">
          ...
          [#list squads as squad]
           <tr>
            <td>${squad.code}</td>
            <td><div title="${squad.name}" style="overflow: hidden;text-overflow: ellipsis;max-width: 200px;display: inline-block;white-space: nowrap;">${squad.name}</div></td>
            <td>${squad.grade}</td>
            <td>${(squad.level.name)!}</td>
            <td>${squad.department.name}</td>
            <td><div title="${(squad.major.name)!} ${(squad.direction.name)!}" style="overflow: hidden;text-overflow: ellipsis;max-width: 200px;display: inline-block;white-space: nowrap;">${(squad.major.name)!} ${(squad.direction.name)!}</div></td>
